def  input param pidUsuario     as char.
def  input param ptoken         as char.
def output param psenhaCorreta  as log.

psenhaCorreta = no.
def new global shared var spid as int.
def new global shared var spidseq as int.

def var pnomeapi as char        init "vendas-token".
def var pnomerecurso as char    init "verifica".
...
hsaida   = temp-table ttusuario:HANDLE.
...
/* HOST API */
    ...
    then vchost = "10.145.0.60". 
    else vchost = "10.2.0.221". 

    vapi = "http://\{IP\}/tslebes/api/vendas/token/verifica".
...
then do:
    ...
    then do:
        ...
        find first ttusuario no-error.
        if avail ttusuario
        then do:
            psenhaCorreta = ttusuario.senhaCorreta.
        end. 
